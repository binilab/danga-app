...
export function Footer() {
  return (
    <footer className="mt-14 border-t border-[var(--line)] bg-white/70">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-3 px-4 py-6 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <p className="text-xs text-slate-500">
          © {new Date().getFullYear()} DANGA. 단번에 가자, 나이스 패션이다.
        </p>
        <nav className="flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-600">
          {policyLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full border border-transparent px-3 py-1 transition hover:border-[var(--line)] hover:bg-slate-50"
            >
              {link.label}
            </Link>
          ))}
