# DANGA 진행사항

## 2026-02-15 Part 1 뼈대 작업 기록

### 1) 라우트/페이지 뼈대 생성
- `app/page.tsx` 랜딩 페이지 구성
- `app/feed/page.tsx` 피드 페이지 생성
- `app/rank/page.tsx` 랭킹 페이지 생성
- `app/me/page.tsx` 마이페이지 placeholder 생성
- `app/post/new/page.tsx` 업로드 화면 placeholder 생성
- `app/p/[id]/page.tsx` 상세 동적 라우트 생성
- `app/admin/page.tsx` 관리자 placeholder 생성

### 2) 공통 레이아웃 정리
- `app/layout.tsx`
  - 전역 메타데이터를 DANGA 컨셉에 맞게 수정
  - 공통 헤더를 전체 페이지에 적용
  - 본문 최대 폭/여백 공통화

### 3) 공통 컴포넌트 분리
- `components/Header.tsx`
  - 좌측 로고, 가운데 메뉴(Feed/Rank), 우측 CTA(시작하기)
  - 모바일에서도 줄바꿈이 자연스럽도록 `flex-wrap` 반영
  - 현재 경로 기준 활성 메뉴 스타일 반영
- `components/PostCard.tsx`
  - 피드/랜딩/상세에서 재사용 가능한 게시글 카드 컴포넌트
- `components/PageTitle.tsx`
  - 페이지 타이틀/설명 영역 공통화

### 4) 더미 데이터 분리
- `lib/mock.ts`
  - 게시글 12개 더미 데이터 작성
  - 랭킹 데이터 제공
  - 상세 페이지 조회용 `getPostById` 함수 작성

### 5) 스타일/디자인 톤 정리
- `app/globals.css`
  - 빠르고 경쾌한 느낌의 색상 변수 정의
  - 공통 카드 스타일(`.danga-panel`) 정의

### 6) 문서 업데이트
- `README.md`
  - 프로젝트 소개, 구현 범위, 실행 방법, 폴더 구조 업데이트
- `docs/진행사항.md`
  - 본 문서 생성 및 작업 상세 로그 기록 시작

## 2026-02-15 랜딩 페이지 고도화(UI 전용) 작업 기록

### 1) 랜딩 섹션 분리 및 조립 구조 변경
- `app/page.tsx`
  - 랜딩 페이지를 섹션 조립 전용 파일로 변경
  - 섹션 순서: Hero -> HowItWorks -> SampleGallery -> RankPreview -> CTA

### 2) 랜딩 전용 컴포넌트 추가
- `components/landing/Hero.tsx`
  - 메인 카피 `단번에 가자`
  - 서브 카피 `코디 올리고, 투표로 바로 평가받자.`
  - CTA 버튼 2개
    - `지금 올리기` -> `/post/new`
    - `구경하기` -> `/feed`
  - 더미 지표(코디 수, 투표 수, 댓글 수) 표시
- `components/landing/HowItWorks.tsx`
  - 3단계 카드(올린다/투표받는다/랭킹 오른다)
- `components/landing/SampleGallery.tsx`
  - 더미 카드 10개 렌더링
  - 각 카드에 이미지 placeholder, 투표수, 댓글수, 작성자 닉네임 표시
  - 이미지는 `next/image` 사용
- `components/landing/RankPreview.tsx`
  - 이번 주 Top 10 미리보기
  - `상위 1%` 배지 예시 카드 포함
- `components/landing/CTA.tsx`
  - 하단 대형 카피 + 단일 CTA 버튼 구성

### 3) 더미 데이터 확장
- `lib/mock.ts`
  - `landingHowSteps` 추가
  - `landingGalleryItems` 추가(10개)
  - `weeklyTopRankers` 추가(Top 10)
  - `createPlaceholderImage` 추가
    - 로컬 이미지 없이도 쓸 수 있는 SVG data URL 생성 함수

### 4) 헤더/UI 톤 개선
- `components/Header.tsx`
  - 기존 구조 유지하면서 스타일을 더 정돈된 느낌으로 미세 조정
  - hover 반응/활성 상태를 명확하게 보이도록 개선
- `app/globals.css`
  - 과한 그라데이션 대신 은은한 배경 톤으로 정리

### 5) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

### 6) 미구현/다음 단계 메모
- 인증/권한(Supabase Auth) 미연동
- 실제 업로드/투표/댓글/신고 기능 미연동
- 무한스크롤 미구현(현재 `더 보기` 버튼)
- 관리자 기능은 placeholder 상태

## 2026-02-15 Part 3 Auth + Header 상태 반영 작업 기록

### 1) Supabase 클라이언트 설정
- `lib/supabase/client.ts`
  - 브라우저용 Supabase 클라이언트 싱글턴 생성
  - `.env.local` 환경 변수 유효성 검사 추가
- `lib/supabase/server.ts`
  - 서버용 Supabase 클라이언트 생성
  - App Router 쿠키 저장/갱신 처리 포함

### 2) OAuth callback 라우트 추가
- `app/auth/callback/route.ts`
  - OAuth 인증 후 `code`를 세션으로 교환
  - 로그인 취소/실패 시 `authMessage`를 URL로 전달해 사용자 안내
  - 외부 URL 리다이렉트 방지를 위해 내부 경로만 허용

### 3) 로그인 모달 구현
- `components/auth/LoginModal.tsx`
  - Header `시작하기` 버튼 클릭 시 모달 오픈
  - Google / Kakao 로그인 버튼 제공
  - 로그인 실패 시 모달 내 안내 문구 표시
  - Kakao 미설정 시 확인 문구 표시

### 4) 로그인 상태 헤더 반영
- `components/Header.tsx`
  - 세션 확인 후 상태별 UI 분기
    - 비로그인: `시작하기` 버튼
    - 로그인: 원형 프로필 버튼 + 드롭다운
  - `profiles` 조회 + (로그인 시) upsert 처리로 닉네임/아바타 동기화
  - 인증 에러 메시지 토스트 안내 반영
- `components/auth/ProfileMenu.tsx`
  - 드롭다운 메뉴 구성
    - 내 피드(`/me`)
    - 업로드(`/post/new`)
    - 로그아웃

### 5) 로그아웃 구현
- Header 드롭다운의 `로그아웃` 클릭 시
  - Supabase 세션 종료
  - 랜딩(`/`)으로 이동

### 6) profiles 테이블 가이드 추가
- `docs/sql/profiles.sql`
  - `profiles` 테이블 생성
  - RLS 정책(본인 조회/생성/수정)
  - `updated_at` 자동 갱신 트리거

### 7) 품질 검증 메모
- lint/build 검증 예정
- local dev에서 OAuth 동작은 Supabase Provider 설정 상태에 따라 달라짐

## 2026-02-15 Part 4 Cloudflare R2 업로드 파이프라인 작업 기록

### 1) 서버 전용 R2 유틸 구현
- `lib/r2.ts`
  - `@aws-sdk/client-s3` 기반 R2 S3Client 설정
  - endpoint: `https://${R2_ACCOUNT_ID}.r2.cloudflarestorage.com`
  - `region: auto` 적용
  - env에서 자격 증명 로드
  - `PutObjectCommand`로 업로드
  - key 생성 규칙 구현
    - `posts/{userId}/{Date.now()}-{random}.{ext}`
  - 파일명 확장자 sanitize + content-type 기반 확장자 보정
  - userId sanitize 처리

### 2) 업로드 API 라우트 구현
- `app/api/upload/route.ts`
  - `POST multipart/form-data` 처리 (`file` 필드)
  - 허용 타입 제한: `image/png`, `image/jpeg`, `image/webp`
  - 최대 용량 5MB 제한 (초과 시 413)
  - 세션 사용자 조회 후 key userId 반영
    - 로그인 사용자는 `supabase auth user.id`
    - 미로그인은 `anon`
  - 성공 응답
    - `{ ok: true, key, url }`
  - 예외 처리 + status code 분기
    - 400: 파일 누락/형식 오류
    - 415: 지원하지 않는 MIME
    - 413: 용량 초과
    - 500: 서버 내부 오류

### 3) /post/new 업로더 UI 구현
- `components/post/ImageUploader.tsx` 추가
  - 파일 선택
  - 업로드 버튼
  - 업로드 중 로딩 상태 + 버튼 비활성화
  - 성공 시 미리보기 이미지 표시
  - 업로드 URL 텍스트 + 복사 버튼
  - 실패 시 사용자 친화적 에러 표시
- `app/post/new/page.tsx`
  - 기존 입력 폼 위에 `ImageUploader` 조립

### 4) 보안/구조 메모
- R2 접근은 API 라우트(`app/api/upload/route.ts`)에서만 수행
- 클라이언트에는 R2 키/시크릿이 전달되지 않음
- 서버/클라이언트 컴포넌트 분리
  - 페이지(`app/post/new/page.tsx`)는 서버 컴포넌트 유지
  - 업로더(`components/post/ImageUploader.tsx`)는 클라이언트 컴포넌트

### 5) 품질 검증 메모
- lint/build 검증 예정
- 실제 업로드 확인은 `.env.local`의 R2 값이 올바를 때 가능

## 2026-02-16 Part 5 posts 테이블 + 작성/피드/상세 구현 기록

### 1) Supabase migration 추가 (posts)
- `supabase/migrations/20260216_create_posts.sql`
  - `public.posts` 테이블 생성
    - `id`, `user_id`, `image_url`, `image_key`, `caption`, `created_at`, `updated_at`, `deleted_at`
  - soft delete 기본 구조(`deleted_at`) 반영
  - 인덱스 추가
    - 최신순 조회용 partial index (`deleted_at is null`)
    - user_id + created_at index
    - deleted_at index
  - RLS/policies 추가
    - select: 공개 조회 가능(soft delete 제외)
    - insert/update: 본인 글만 허용
  - `updated_at` 자동 갱신 트리거 추가
- 실행 편의용 SQL 복사본
  - `docs/sql/posts.sql`

### 2) 게시글 생성 API 추가
- `app/api/posts/route.ts`
  - `POST` 요청으로 게시글 생성
  - 요청 본문: `imageUrl`, `imageKey`, `caption`
  - 세션 사용자 확인 후 `user_id`를 서버에서 강제 적용
  - 유효성 검증
    - 이미지 URL 필수
    - 캡션 필수 / 500자 제한
  - 저장 성공 시 `{ ok: true, id }` 반환

### 3) /post/new 작성 흐름 완성
- `components/post/ImageUploader.tsx`
  - 업로드 성공 후 캡션 입력 UI 제공
  - `게시글 등록` 버튼으로 `POST /api/posts` 호출
  - 저장 성공 시 `/feed` 이동
- `app/post/new/page.tsx`
  - 기존 더미 폼 제거, 실제 작성 흐름 중심으로 정리

### 4) /feed 조회 구현
- `app/feed/page.tsx`
  - `posts` 테이블 최신순 조회
  - `deleted_at is null` 조건 적용
  - 20개 로딩 + `더보기` 버튼(page query)
  - 이미지는 `image_key` 기준 서명 URL 우선 표시(실패 시 `image_url` fallback)
- `components/post/PostItemCard.tsx`
  - 피드/상세 공용 카드 컴포넌트 추가

### 5) /p/[id] 상세 구현
- `app/p/[id]/page.tsx`
  - posts 단건 조회
  - `deleted_at is null` 조건 적용
  - 조회 실패 시 `notFound()` 처리

### 6) 공용 타입/유틸 추가
- `lib/posts.ts`
  - `PostRow` 타입
  - 작성자 라벨/날짜 포맷 유틸

### 7) 품질 검증 메모
- lint/build 검증 예정
- 운영 확인 목표
  - 로그인 -> 이미지 업로드 -> 게시글 생성 -> feed 노출

## 2026-02-16 Part 6 투표(좋아요) MVP 구현 기록

### 1) votes 토글 API 추가
- `app/api/votes/route.ts`
  - `POST`: 좋아요 생성
  - `DELETE`: 좋아요 취소
  - 로그인 사용자 검사
  - `postId` 유효성(UUID 형식) 검사
  - 응답에 `likedByMe`, `count` 반환
  - 중복 좋아요(`unique(post_id, voter_id)`) 충돌은 정상 처리

### 2) 좋아요 로직 재사용 훅 분리
- `hooks/useVote.ts`
  - optimistic UI 토글
  - 실패 시 원복 + 안내 메시지
  - 중복 연타 방지(`isPending`)
  - 비로그인 시 로그인 모달 오픈 이벤트(`danga:open-login`) 전송

### 3) votes 집계 유틸 추가
- `lib/votes.ts`
  - `postIds` 기준 votes를 `in()`으로 한 번에 조회
  - `count` + `likedByMe` 집계 맵 생성
  - N+1 조회 방지

### 4) UI 반영
- `components/post/VoteButton.tsx`
  - 하트 버튼 + 좋아요 수 표시
  - 활성/비활성 스타일 분리
  - 오류 메시지 표시
- `components/post/PostItemCard.tsx`
  - 카드 하단에 좋아요 버튼 배치
  - 링크 클릭 영역과 버튼 영역 분리(상호작용 충돌 방지)

### 5) /feed 반영
- `app/feed/page.tsx`
  - posts 20개 조회 후 post ids 추출
  - votes를 한 번에 조회해 카드별 좋아요 정보 주입
  - 로그인 여부에 따라 버튼 동작 분기

### 6) /p/[id] 반영
- `app/p/[id]/page.tsx`
  - 상세 단건에도 좋아요 집계/토글 동일 적용

### 7) 헤더 로그인 모달 연동
- `components/Header.tsx`
  - `danga:open-login` 이벤트 수신 시 로그인 모달 오픈
  - 비로그인 사용자의 좋아요 클릭에서 모달 유도 가능

### 8) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

### 9) 운영 확인 체크포인트
- 로그인 상태에서 feed/상세 좋아요 토글 가능
- 새로고침 후 좋아요 상태/카운트 일치
- 비로그인 좋아요 클릭 시 로그인 모달 유도

## 2026-02-16 Part 6 votes 테이블 SQL 추가 기록

### 1) Supabase migration 추가 (votes)
- `supabase/migrations/20260216_create_votes.sql`
  - `public.votes` 테이블 생성
    - `post_id`, `voter_id`, `created_at`
  - 기본키 `(post_id, voter_id)`로 중복 좋아요 방지
  - 인덱스 추가
    - `voter_id`
    - `created_at desc`
  - RLS/policies 추가
    - select all
    - insert own
    - delete own

### 2) 실행 편의용 SQL 문서 추가
- `docs/sql/votes.sql`
  - migration과 동일한 내용 제공

### 3) 문서 반영
- `README.md`
  - 사전 설정 단계에 `docs/sql/votes.sql` 실행 절차 추가
  - 폴더 구조에 `docs/sql/votes.sql`, `supabase/migrations/20260216_create_votes.sql` 반영

## 2026-02-16 Part 6 votes 테이블 SQL 추가 기록

### 1) Supabase migration 추가 (votes)
- `supabase/migrations/20260216_create_votes.sql`
  - `public.votes` 테이블 생성
    - `post_id`, `voter_id`, `created_at`
  - 기본키 `(post_id, voter_id)`로 중복 좋아요 방지
  - 인덱스 추가
    - `voter_id`
    - `created_at desc`
  - RLS/policies 추가
    - select all
    - insert own
    - delete own

### 2) 실행 편의용 SQL 문서 추가
- `docs/sql/votes.sql`
  - migration과 동일한 내용 제공

### 3) 문서 반영
- `README.md`
  - 사전 설정 단계에 `docs/sql/votes.sql` 실행 절차 추가
  - 폴더 구조에 `docs/sql/votes.sql`, `supabase/migrations/20260216_create_votes.sql` 반영

## 2026-02-16 Part 8 주간/월간 랭킹 + 퍼센타일 뱃지 구현 기록

### 1) 1.md 기준 확인 후 구현
- `docs/1.md` Part 8 목표/집계 전략을 기준으로 반영
- 랭킹 조회 소스
  - `weekly_post_rankings`
  - `monthly_post_rankings`

### 2) /rank 페이지 실데이터 구현
- `app/rank/page.tsx`
  - 주간/월간 탭 UI (`?period=weekly|monthly`)
  - 선택 탭의 뷰에서 Top 50 조회(rank asc)
  - N+1 방지 조회 전략 적용
    - ranking view 조회 -> post_id 목록 확보
    - posts를 `in(id, post_ids)`로 일괄 조회
    - profiles를 `in(id, user_ids)`로 일괄 조회
  - 항목 표시
    - rank number
    - badge pill
    - score
    - thumbnail
    - 작성자 nickname/avatar
  - 항목 클릭 시 `/p/[id]` 이동

### 3) 랭킹 유틸 추가
- `lib/rankings.ts`
  - period 파싱/뷰 이름 변환
  - badge 라벨/스타일 유틸
  - post_id 목록 기반 뱃지 일괄 조회 유틸

### 4) (선택) /feed + /p/[id] 카드 뱃지 반영
- `app/feed/page.tsx`
  - 현재 페이지의 post_id들만 대상으로 주간 뱃지 조회(in)
  - 카드에 badge 전달
- `app/p/[id]/page.tsx`
  - 상세 단건도 주간 뱃지 조회 후 표시
- `components/post/PostItemCard.tsx`
  - badge pill 렌더링 추가

### 5) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

### 6) 사용자(운영) 확인 항목
- Supabase에서 `weekly_post_rankings`, `monthly_post_rankings` 뷰 생성 완료 필요
- `/rank` 탭 전환 시 데이터가 실제 votes 기반으로 바뀌는지 확인
- 배포 환경에서 RLS/권한 문제 없이 랭킹 조회 가능한지 확인

## 2026-02-16 Part 9 /me 마이페이지 고도화 구현 기록

### 1) /me 탭 UI 및 데이터 로딩 구현
- `app/me/page.tsx`
  - 탭 UI 구성: `내 글` / `내 좋아요` / `내 댓글`
  - 로그인 사용자 기준으로 탭별 데이터 조회
  - 비로그인 상태는 안내 메시지 표시
  - 탭별 오류 상태 UI 분기
- `app/me/loading.tsx`
  - 서버 데이터 로딩 중 스켈레톤 UI 제공

### 2) 내 글 데이터 로딩
- 조회 조건
  - `posts.user_id = auth.uid()`
  - `posts.deleted_at is null`
  - 최신순 + 20개 단위(+1 조회로 더보기 여부 판단)
- N+1 방지
  - 현재 페이지 post ids 기준 votes를 `in()`으로 1회 조회해 count/likedByMe 매핑
  - 주간 badge도 `in()`으로 1회 조회
- 컴포넌트
  - `components/me/MyPosts.tsx`

### 3) 내 좋아요 데이터 로딩
- 조회 조건
  - `votes.voter_id = auth.uid()`
  - 최신순 50개
- N+1 방지
  - votes에서 post_id 목록 확보 후 `posts.in(id, post_ids)` 1회 조회
  - votes 순서를 기준으로 최종 리스트 매핑
- 컴포넌트
  - `components/me/MyLikes.tsx`

### 4) 내 댓글 데이터 로딩
- 조회 조건
  - `comments.user_id = auth.uid()`
  - `comments.deleted_at is null`
  - 최신순 50개
- N+1 방지
  - comments의 post_id 목록으로 `posts.in(id, post_ids)` 1회 조회
  - 댓글 카드에 post thumbnail/caption 결합
- 컴포넌트
  - `components/me/MyComments.tsx`

### 5) 프로필 편집 + 탈퇴(soft delete)
- `components/me/ProfileEditor.tsx`
  - nickname, avatar_url 수정
  - 저장 성공/실패 안내 메시지
  - 저장 후 `router.refresh()` + 헤더 갱신 이벤트 전송
- `components/me/AccountDangerZone.tsx`
  - 탈퇴 확인(confirm)
  - `profiles.deleted_at = now()` 업데이트
  - `supabase.auth.signOut()` 후 `/` 이동
- `components/Header.tsx`
  - `danga:refresh-header-user` 이벤트 수신 시 헤더 프로필 상태 재동기화

### 6) Supabase 작업 (MCP 사용)
- 로컬 migration 추가
  - `supabase/migrations/20260216_profiles_soft_delete.sql`
  - `profiles.deleted_at` 컬럼/인덱스 보강
- 문서 SQL 반영
  - `docs/sql/profiles.sql`에 `deleted_at` + 인덱스 추가
- MCP 실제 반영
  - `part9_profiles_soft_delete` migration을 Supabase에 적용(`success: true`)

### 7) 문서 반영
- `README.md`
  - Part 9 기능(마이페이지 탭/프로필 편집/탈퇴) 설명 추가
  - 폴더 구조에 `components/me/*`, `app/me/loading.tsx`, 신규 migration 반영

## 2026-02-16 Part 10 관리자(Admin) + 신고(Reports) MVP 구현 기록

### 1) 신고 기능(유저) 구현
- `app/api/reports/route.ts`
  - 게시글/댓글 신고 생성 API 추가
  - 입력 검증
    - `targetType`: `post | comment`
    - `targetId`: UUID 형식
    - `reason`: 5자~300자
  - 로그인 사용자만 신고 허용(`reporter_id = auth.uid()`)
  - 중복 신고(unique) 오류를 `이미 신고했습니다.`로 안내
- `components/report/ReportModal.tsx`
  - 신고 사유 입력 모달
  - 성공/실패 안내 문구 처리
  - 비로그인 상태에서 신고 시 로그인 모달 유도 이벤트 전송
- `components/report/ReportButton.tsx`
  - 신고 버튼 + 모달 오픈 래퍼
  - 신고 성공 후 버튼 하단 안내 문구 표시
- `app/p/[id]/page.tsx`
  - 게시글 신고 버튼 추가
  - 댓글 목록 렌더링 + 댓글별 신고 버튼 추가

### 2) /admin 접근 제어 구현
- `app/admin/page.tsx`
  - 세션 확인
  - 본인 `profiles.role` 조회
  - `role='admin'` + `deleted_at is null` 조건으로 접근 제어
  - 비로그인/비관리자 계정은 접근 불가 안내 UI 표시

### 3) /admin Reports 화면 구현
- `components/admin/AdminReportsClient.tsx`
  - 신고 목록 상태 관리
  - status 필터링
  - 상태 업데이트 / 조치 RPC 실행
- `components/admin/ReportsTable.tsx`
  - 신고 리스트 UI (status 필터 + 선택)
  - `created_at desc` 기준 정렬된 목록 표시
- `components/admin/ReportDetail.tsx`
  - 선택된 신고 상세(reason/notes/status) 표시
  - 상태 저장 버튼
  - 조치 버튼
    - post -> `admin_soft_delete_post`
    - comment -> `admin_soft_delete_comment`

### 4) 데이터 로직 분리
- `lib/reports.ts`
  - 신고 도메인 타입/상수/검증/상태 라벨 유틸
- `lib/admin.ts`
  - admin 권한 판별
  - reports + target(posts/comments) 결합 매핑
  - 리뷰 업데이트 payload 생성

### 5) Supabase 작업(MCP 사용)
- 실제 DB에 reports/admin RPC 구성 반영
  - migration name: `part10_reports_admin_mvp`
  - 적용 결과: success
- 확인 완료
  - `public.reports` 테이블 생성
  - RPC 함수 생성
    - `admin_soft_delete_post`
    - `admin_soft_delete_comment`
    - `is_admin`
  - reports RLS 정책 생성
    - `reports_select_own`
    - `reports_insert_own`
    - `reports_select_admin_all`
    - `reports_update_admin`
- 로컬 migration/문서 SQL 추가
  - `supabase/migrations/20260216_create_reports_admin.sql`
  - `docs/sql/reports.sql`

### 6) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

## 2026-02-16 UI 개선 스프린트 기록 (기능/DB/라우팅 변경 없음)

### 1) 디자인 시스템 토큰 + 공통 UI 컴포넌트
- `app/globals.css`
  - 간격/타이포/radius/shadow 토큰 추가
  - 공통 타이포 클래스(`danga-title`, `danga-subtitle`) 추가
- `components/ui/*`
  - `Button.tsx` / `Card.tsx` / `Badge.tsx` / `State.tsx` / `Skeleton.tsx` 추가
  - 상태 UI(Empty/Error), 스켈레톤, 버튼 스타일 일관화

### 2) Header/Footer 레이어 정리
- `components/layout/Header.tsx`
  - 기존 인증/세션/알림 로직 유지
  - 시각 톤 통일(간격/터치 타깃/배지 스타일)
  - 비로그인 CTA 카피 변경: `로그인하고 시작`
- `components/layout/Footer.tsx`
  - 정책 링크/브랜드 카피 톤 정리
- `components/Header.tsx`, `components/Footer.tsx`
  - 기존 import 호환을 위한 re-export 래퍼로 유지
- `app/layout.tsx`
  - layout 컴포넌트 경로로 교체

### 3) Feed 카드 리디자인
- `components/post/PostCard.tsx` 신규
  - 이미지 비율 4:5 고정
  - 캡션/태그/액션바(작성자/날짜/좋아요/배지) 정리
  - 태그 chip 클릭 시 `/search?tag=...` 이동
- `components/post/PostItemCard.tsx`
  - 기존 코드 호환을 위한 `PostCard` re-export로 정리
- `components/post/VoteButton.tsx`
  - 액션바와 어울리도록 버튼 크기/터치 영역 정리
- `app/feed/page.tsx`
  - `PostCard` 사용
  - Empty/Error를 공통 State 컴포넌트로 교체

### 4) Detail(/p/[id]) 레이아웃 가독성 개선
- `app/p/[id]/page.tsx`
  - 메인 카드 + 보조 정보 패널 2열 구조로 정리(모바일은 1열)
  - 작성자/반응 정보를 우측 보조 카드에 분리
- `components/comment/CommentSection.tsx`
  - 입력/목록 카드화
  - 댓글 placeholder 카피 변경: `한 줄 피드백 남겨줘`
  - Empty 상태를 공통 `EmptyState`로 교체

### 5) /post/new 단계형 UI
- `components/post/ImageUploader.tsx`
  - 1단계(이미지 업로드) / 2단계(코디 정보 입력) 구조화
  - 업로드중/저장중/오류 상태를 명확한 카드/메시지로 분리
  - 등록 버튼 카피 변경: `코디 올리기`
- `app/post/new/page.tsx`
  - 페이지 타이틀/설명 카피 톤 정리

### 6) Empty/Loading/Error 공통화 적용
- `app/feed/page.tsx`, `app/search/page.tsx`, `app/notifications/page.tsx`
  - `EmptyState`, `ErrorState` 적용
- `app/search/loading.tsx`, `app/me/loading.tsx`
  - `Skeleton` 컴포넌트 적용

### 7) 카피 톤 통일
- `components/layout/Header.tsx`
  - `시작하기` -> `로그인하고 시작`
- `components/landing/Hero.tsx`, `components/landing/CTA.tsx`
  - `구경하기` -> `피드 보기`
  - `지금 올리기` -> `코디 올리기`
- `components/auth/ProfileMenu.tsx`
  - 메뉴 `업로드` -> `코디 올리기`
- `components/auth/LoginModal.tsx`
  - 제목/버튼 카피를 로그인 중심 톤으로 정리

### 8) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

## 2026-02-16 Part 15 검색 + 태그 MVP 구현 기록

### 1) 태그 파싱/검증 로직 분리
- `lib/tags.ts`
  - `#` 제거 + 공백/쉼표 split + trim + 중복 제거
  - 게시글 태그 규칙 검증
    - 최대 5개
    - 각 1~20자
  - query `tag` 단일값 정규화 유틸 추가

### 1-1) posts.tags 컬럼 추가 및 MCP 반영
- 로컬 SQL 추가
  - `docs/sql/posts_tags.sql`
  - `supabase/migrations/20260216_add_posts_tags.sql`
- 적용 내용
  - `posts.tags text[] not null default '{}'`
  - `posts_tags_gin_idx` (GIN) 인덱스 추가
- MCP 실제 반영
  - migration name: `part15_posts_tags`
  - 적용 결과: success
  - 확인
    - `public.posts.tags` 컬럼 존재
    - `posts_tags_gin_idx` 인덱스 존재

### 2) 게시글 작성(/post/new) 태그 입력 추가
- `components/post/ImageUploader.tsx`
  - 캡션 아래 태그 입력 UI 추가
  - 입력 예시 문구 제공: `#미니멀 #스트릿` / `미니멀, 스트릿`
  - 등록 직전 태그 파싱/검증 후 `tags` 배열을 API에 전달
  - 파싱된 태그 chip 미리보기 추가

### 3) posts insert 시 tags 저장
- `app/api/posts/route.ts`
  - payload에 `tags` 필드 처리 추가
  - 서버에서도 동일 태그 검증 로직 적용
  - `posts.insert` 시 `tags` 배열 저장

### 4) /search 페이지 구현
- `app/search/page.tsx`
 - `app/search/loading.tsx`
  - route: `/search?q=...&tag=...&page=...`
  - 조회 조건
    - `deleted_at is null`
    - `q` 존재 시 `caption ilike`
    - `tag` 존재 시 `tags @> [tag]` (`contains`)
    - `q` + `tag` 동시 입력 시 AND
  - 최신순 + 20개 페이징 + 더보기 링크 구현
  - 결과 카드 렌더링은 기존 `PostItemCard` 재사용

### 5) 검색 입력 UX
- `components/search/SearchForm.tsx`
  - Enter 제출 가능한 공통 검색 폼 컴포넌트 추가
  - 활성 태그 표시/해제 UI 제공
- `app/feed/page.tsx`
  - 페이지 상단 검색 입력 추가
- `app/search/page.tsx`
  - 상단 검색바 동일 컴포넌트로 제공

### 6) 태그 chip 표시 + 클릭 검색 이동
- `components/post/PostItemCard.tsx`
  - 태그 chip 렌더링 추가
  - chip 클릭 시 `/search?tag=태그` 이동
- 태그 데이터 전달
  - `app/feed/page.tsx`
  - `app/p/[id]/page.tsx`
  - `components/me/MyPosts.tsx` + `app/me/page.tsx`
  - `lib/posts.ts`의 `PostRow`에 `tags` 필드 반영

### 7) 기타 반영
- `app/rank/page.tsx`, `app/sitemap.ts`
  - posts select에 tags 포함
  - sitemap에 `/search` 추가
- 문서 업데이트
  - `README.md` Part 15 반영

### 8) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

## 2026-02-16 댓글 작성 누락 보완(핫픽스) 기록

### 1) 문제
- `/p/[id]` 상세에서 댓글 목록만 있고, 댓글 작성 UI/API가 없어
  `아직 댓글이 없습니다.`만 표시되는 상태였음

### 2) 수정 내용
- 댓글 도메인 유틸 추가
  - `lib/comments.ts`
  - 댓글 길이 제한/검증(1~500자)
- 댓글 생성 API 추가
  - `app/api/comments/route.ts`
  - `postId` UUID 검사, 로그인 검사, 대상 게시글 존재 검사
  - 성공 시 생성된 댓글 row 반환
- 상세 페이지 댓글 영역 클라이언트 컴포넌트로 교체
  - `components/comment/CommentSection.tsx`
  - 댓글 작성 textarea + `댓글 달기` 버튼
  - 등록 성공 시 목록 상단 즉시 반영
  - 비로그인 작성 시 로그인 모달 유도
- 기존 상세 페이지 연결
  - `app/p/[id]/page.tsx`
  - 서버에서 조회한 초기 댓글을 `CommentSection`으로 전달

### 3) 문서 반영
- `README.md`
  - 댓글 MVP(작성 폼/즉시 반영) 항목 추가
  - 폴더 구조에 `app/api/comments/route.ts`, `components/comment/CommentSection.tsx` 반영

## 2026-02-16 Part 12(선택) AES-256-GCM 암호화 유틸 + profiles 암호화 컬럼 적용 기록

### 1) 서버 전용 암호화 유틸 추가
- `lib/crypto/aesgcm.ts`
  - `server-only` 모듈로 고정
  - `APP_ENCRYPTION_KEY`(base64, 32bytes) 검증
  - `encrypt(plaintext) -> ciphertext(base64)` 구현
  - `decrypt(ciphertext) -> plaintext` 구현
  - 포맷: `iv(12) + tag(16) + cipher`를 하나의 base64 문자열로 패킹
- `lib/crypto/profileSensitive.ts`
  - profiles 민감 필드(email/name) 암복호화 헬퍼 제공

### 2) profiles 암호화 저장 패턴 적용
- `app/api/profiles/encrypted/route.ts`
  - 로그인 사용자 기준으로 email/name을 서버에서 암호화 후 저장
  - `profiles.email_enc`, `profiles.name_enc`에 암호문 저장
  - 신규 row 생성 가능하도록 fallback nickname 처리
  - 에러 시 상태코드/메시지 반환
- `components/Header.tsx`
  - 로그인/세션 갱신 시 `/api/profiles/encrypted` 호출
  - 민감 정보 동기화 실패 안내 메시지 처리

### 3) admin에서만 복호화 표시
- `app/admin/page.tsx`
  - admin 권한 확인 이후에만 reporter 민감 정보 조회 RPC 호출
  - 서버에서만 복호화 수행 후 UI 데이터로 전달
  - 복호화 실패 시 안전 fallback(닉네임/익명 라벨)
- `lib/admin.ts`
  - 관리자 신고 아이템에 `reporterName`, `reporterEmail` 필드 추가
- `components/admin/ReportsTable.tsx`
- `components/admin/ReportDetail.tsx`
  - admin 화면에서 신고자 이름/이메일 표시

### 4) Supabase 스키마/MCP 반영
- 로컬 migration 추가
  - `supabase/migrations/20260216_profiles_encryption.sql`
  - 내용
    - `profiles.email_enc`, `profiles.name_enc` 컬럼 추가
    - `admin_list_profile_identities(uuid[])` security definer RPC 추가
- 문서 SQL 추가
  - `docs/sql/profiles_encryption.sql`
  - `docs/sql/profiles.sql`에 암호화 컬럼/코멘트 반영
- MCP 실제 반영
  - migration name: `part12_profiles_encryption`
  - 적용 결과: success
  - 확인
    - `profiles.email_enc`, `profiles.name_enc` 컬럼 존재
    - `admin_list_profile_identities` 함수 존재

### 5) 보안 메모
- 암호화 코드는 서버 전용(`server-only`)으로 클라이언트 번들 제외
- 키는 `process.env.APP_ENCRYPTION_KEY`에서만 로드
- DB 암호화 컬럼에는 평문 대신 암호문(base64) 저장

## 2026-02-16 Part 13 런칭 품질(SEO/성능/정책 문서/QA) 마무리 기록

### 1) SEO 메타데이터 강화
- `app/layout.tsx`
  - `metadataBase`, `title`, `description` 기본값 강화
  - `openGraph`, `twitter` 메타데이터 추가
  - `openGraph.images`, `twitter.images` 연결
- `app/opengraph-image.tsx`
- `app/twitter-image.tsx`
  - 공유 시 사용할 기본 미리보기 이미지를 서버에서 동적 생성
- `app/admin/page.tsx`
  - `metadata.robots`에 `index: false`, `follow: false` 설정(noindex)

### 2) robots/sitemap 제공
- `app/robots.ts`
  - `Disallow: /admin`, `Disallow: /api`
  - sitemap 경로 고지
- `app/sitemap.ts`
  - 주요 정적 라우트(`/`, `/feed`, `/rank`, `/privacy`, `/terms`, `/contact` 등) 제공

### 3) 이미지 최적화/도메인 허용
- `next.config.ts`
  - `images.remotePatterns` 설정
  - 허용 도메인
    - `img.danga.site`
    - `*.r2.cloudflarestorage.com`
    - `R2_PUBLIC_BASE_URL` 호스트(동적)
- `components/post/PostItemCard.tsx`
  - 피드/상세 카드 이미지 렌더링을 `next/image`로 전환

### 4) 정책 페이지 + Footer 링크
- 정책 페이지 생성
  - `app/privacy/page.tsx`
  - `app/terms/page.tsx`
  - `app/contact/page.tsx`
- 공통 푸터 추가
  - `components/Footer.tsx`
  - `app/layout.tsx`에 전역 Footer 조립

### 5) 운영 로그/에러 메시지 정리
- `lib/log.ts`
  - 개발 환경에서만 서버 에러 로그 출력하도록 공통 유틸 추가
- `console.error` 직접 호출 제거/치환
  - `lib/votes.ts`
  - `lib/rankings.ts`
  - `app/api/posts/route.ts`
  - `app/api/upload/route.ts`
  - `app/api/votes/route.ts`
  - `app/api/profiles/encrypted/route.ts`
- 사용자 안내 메시지는 기술 상세 대신 친화적인 문구 중심으로 유지

### 6) 문서 반영
- `README.md`
  - Part 13 기능 요약 반영
  - 환경 변수/폴더 구조/정책 페이지/robots/sitemap 반영

### 7) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과

## 2026-02-16 Part 14 In-app Notifications MVP 구현 기록

### 1) 알림 공통 로직 분리
- `lib/notifications.ts`
  - 알림 타입/아이템 타입 정의
  - unread count 조회 유틸 `fetchUnreadNotificationCount`
  - 알림 목록 + actor profile 일괄 결합 유틸 `fetchNotificationItemsForUser`
  - 알림 타입 라벨/메시지/이동 링크 계산 유틸 제공

### 1-1) Supabase 스키마 생성 및 MCP 반영
- 로컬 SQL 추가
  - `docs/sql/notifications.sql`
  - `supabase/migrations/20260216_create_notifications.sql`
- 생성 대상
  - `public.notifications` 테이블
  - RLS 정책(`select/update/delete own`)
  - RPC 함수 `notify_vote`, `notify_comment`
- MCP 실제 반영
  - migration name: `part14_notifications_mvp`
  - 적용 결과: success
  - 확인
    - `public.notifications` 테이블 존재
    - `notify_vote`, `notify_comment` 함수 존재
    - notifications RLS 정책 3개 존재

### 2) 좋아요/댓글 생성 시 알림 RPC 호출 연결
- `app/api/votes/route.ts`
  - 좋아요 insert 성공 시 `notify_vote(p_post_id, p_actor_id)` 호출
  - unique 충돌(`23505`, 이미 좋아요)은 알림 중복 생성 없이 정상 응답 유지
  - 알림 RPC 실패는 사용자 액션 실패로 처리하지 않고 dev 로그만 기록
- `app/api/comments/route.ts`
  - 댓글 insert 성공 시 `notify_comment(p_post_id, p_comment_id, p_actor_id)` 호출
  - RPC 실패 시 댓글 작성은 성공 처리하고 dev 로그만 기록

### 3) 헤더 알림 아이콘 + unread count
- `components/Header.tsx`
  - 로그인 사용자만 우측에 벨 아이콘 노출
  - `notifications`에서 `is_read=false` count를 조회해 뱃지 표시
  - `danga:notifications-updated` 이벤트 수신 시 카운트 즉시 재동기화
  - 로그아웃/세션 만료 시 카운트 초기화
- `components/auth/ProfileMenu.tsx`
  - 드롭다운 메뉴에 `/notifications` 링크 추가

### 4) /notifications 페이지 구현
- `app/notifications/page.tsx`
  - 로그인 사용자 전용 알림 화면
  - 최신순 알림 로딩
  - 비로그인 시 로그인 안내 UI
- `components/notifications/NotificationList.tsx`
  - 알림 클릭 시 개별 읽음 처리(`is_read=true`)
  - `post_id`가 있으면 `/p/[id]`로 이동
  - `모두 읽음` 일괄 처리 지원
  - 로딩/빈 상태/에러 메시지 처리

### 5) 문서 반영
- `README.md`
  - Part 14 기능 요약 추가
  - 사전 설정에 notifications SQL/RPC 반영 항목 추가
  - 폴더 구조에 notifications 관련 파일 반영

### 6) 품질 검증
- `npm run lint` 통과
- `npm run build` 통과
